...
export class AdsManager {
  ...
  initDesktopBanner() {
    ...
    // Só cria o banner se não existir
    if (!slot.querySelector("ins.adsbygoogle")) {
      if (this.cfg.network === "adsense" && this.cfg.adsenseClientId) {
        ...
        if (this.cfg.adsenseBannerSlotId) {
          ins.setAttribute("data-ad-slot", this.cfg.adsenseBannerSlotId);
        }
  ...
  async maybeShowMobileInterstitial(): Promise<void> {
    ...
    const n =
      (parseInt(localStorage.getItem(KEY_MOBILE_COUNT) ?? "0", 10) || 0) + 1;
    ...
    if (!overlay) {
      ...
      document
        .getElementById("adClose")
        ?.addEventListener("click", () => this.closeInterstitial());
    }
