...
export class MusicManager {
  ...
  init() {
    this.stems.set("keys", {
      key: "keys",
      sound: this.scene.sound.add("music_keys", { loop: true, volume: 0 }),
      targetVolume: 0,
    });
    this.stems.set("bass", {
      key: "bass",
      sound: this.scene.sound.add("music_bass", { loop: true, volume: 0 }),
      targetVolume: 0,
    });
    this.stems.set("drums", {
      key: "drums",
      sound: this.scene.sound.add("music_drums", { loop: true, volume: 0 }),
      targetVolume: 0,
    });
  ...
  public attach(scene: Phaser.Scene) {
    this.scene = scene; // permite reaproveitar a mesma instância em outra cena
  }

  public isStarted(): boolean {
    // usa o 'started' que você já tem
    // (se seu init() já faz o guard, ok também)
    // @ts-ignore (caso esteja private)
